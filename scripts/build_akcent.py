...
AkCent build entrypoint.

Что делает:
- загружает config supplier-layer;
- получает source-offers;
- фильтрует входной поток;
- собирает raw offers;
- пишет raw/final фиды;
- запускает quality gate;
- печатает стабильный summary.
"""
    ...
    *,
    ...
    if "vendor_blacklist" in sig.parameters:
        kwargs["vendor_blacklist"] = {
            str(item).casefold()
            for item in (policy_cfg.get("vendor_blacklist_casefold") or [])
            if str(item).strip()
        }
...
def _load_akcent_qg_callable():
    module = importlib.import_module("suppliers.akcent.quality_gate")
...
def _run_quality_gate(*, out_file: str, raw_out_file: str, policy_cfg: dict[str, Any]) -> None:
    ...
    if "feed_path" in params:
        kwargs = {"feed_path": raw_out_file}
        if "baseline_path" in params:
            kwargs["baseline_path"] = baseline_path
        if "report_path" in params:
            kwargs["report_path"] = report_path
        if "max_new_cosmetic_offers" in params:
            kwargs["max_new_cosmetic_offers"] = max_cosmetic_offers
        if "max_new_cosmetic_issues" in params:
            kwargs["max_new_cosmetic_issues"] = max_cosmetic_issues
        if "max_cosmetic_offers" in params:
            kwargs["max_cosmetic_offers"] = max_cosmetic_offers
        if "max_cosmetic_issues" in params:
            kwargs["max_cosmetic_issues"] = max_cosmetic_issues
        if "enforce" in params:
            kwargs["enforce"] = enforce
        if "freeze_current_as_baseline" in params:
            kwargs["freeze_current_as_baseline"] = freeze_current

        result = qg_callable(**kwargs)
    ...
    if "raw_out_file" in params:
        kwargs["raw_out_file"] = raw_out_file
    if "supplier" in params:
        kwargs["supplier"] = str(policy_cfg.get("supplier") or "AkCent").strip() or "AkCent"
    if "version" in params:
        kwargs["version"] = BUILD_AKCENT_VERSION
    if "baseline_path" in params:
        kwargs["baseline_path"] = baseline_path
    if "report_path" in params:
        kwargs["report_path"] = report_path
