...
BUILD_AKCENT_VERSION = "build_akcent_v70_import_fix"
...
def _load_supplier_config(cfg_dir: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    return (_read_yaml(cfg_dir / FILTER_FILE_DEFAULT), _read_yaml(cfg_dir / SCHEMA_FILE_DEFAULT), _read_yaml(cfg_dir / POLICY_FILE_DEFAULT))
...
def _filter_prefixes_from_cfg(filter_cfg: dict[str, Any]) -> list[str]:
    ...
    raw = include_rules.get("name_prefixes") or filter_cfg.get("name_prefixes") or include_rules.get("allow_name_prefixes") or filter_cfg.get("allow_name_prefixes") or []
...
def _load_akcent_qg_callable():
    ...
    raise RuntimeError("AkCent quality_gate.py не экспортирует run_quality_gate. Найдены callable: " + (", ".join(exported) if exported else "нет"))
...
def _run_quality_gate(*, out_file: str, raw_out_file: str, policy_cfg: dict[str, Any]) -> None:
    ...
    if not bool(qg_cfg.get("enabled", True)):
        return
    qg_callable, qg_name = _load_akcent_qg_callable()
    ...
    if "enforce" in params: kw["enforce"] = enforce
    if not kw:
        raise RuntimeError(f"AkCent quality_gate entrypoint '{qg_name}' не имеет поддерживаемых аргументов.")
    result = qg_callable(**kw)
    if isinstance(result, tuple) and len(result) == 2 and isinstance(result[0], bool):
        ok, summary = result
        if summary: print(summary)
