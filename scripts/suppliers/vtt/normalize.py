...
VENDOR_HINTS = (
    ...
    "Toshiba", "Develop", "Gestetner", "RISO", "Hi-Black",
)
...
# Защищённые брендовые токены: не должны участвовать в color-localization.
_PROTECTED_BRAND_TOKENS: tuple[tuple[re.Pattern[str], str], ...] = (
    (
        re.compile(r"(?iu)\bhi(?:\s*[-–—]?\s*|\s+)black\b"),
        "__VTT_BRAND_HIBLACK__",
    ),
    # Страховка: если где-то раньше уже успел образоваться сломанный вариант.
    (
        re.compile(r"(?iu)\bhi(?:\s*[-–—]?\s*|\s+)ч(?:е|ё)рн(?:ый|ая|ое|ые)?\b"),
        "__VTT_BRAND_HIBLACK__",
    ),
)

_RESTORE_PROTECTED_BRANDS = {
    "__VTT_BRAND_HIBLACK__": "Hi-Black",
}
...
def _protect_brand_tokens(text: str) -> str:
    """Временно защитить брендовые токены от локализации цвета."""
    s = _norm_spaces(text)
    if not s:
        return ""
    for rx, placeholder in _PROTECTED_BRAND_TOKENS:
...
def canon_vendor(vendor: str) -> str:
    ...
    v = _restore_brand_tokens(_protect_brand_tokens(v))
...
def infer_color_from_title(title: str) -> str:
    ...
    # Сначала защищаем брендовые токены, чтобы Black внутри Hi-Black не считался цветом.
    t = _protect_brand_tokens(t)
