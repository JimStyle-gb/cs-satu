...
from suppliers.vtt import builder as _vtt_builder
...
_BUILD_OFFERS = getattr(_vtt_builder, "build_offers", None)
_BUILD_OFFER_FROM_RAW = getattr(_vtt_builder, "build_offer_from_raw", None)

def _batch_build_offers(
    raw_rows,
    *,
    id_prefix: str,
) -> list[OfferOut]:
    """Backward-safe bridge:
    - работает с новым batch-entrypoint build_offers(...),
    - и со старым builder.py, где есть только build_offer_from_raw(...).
    """
    if callable(_BUILD_OFFERS):
        return _BUILD_OFFERS(raw_rows, id_prefix=id_prefix)

    if not callable(_BUILD_OFFER_FROM_RAW):
        raise ImportError(
            "suppliers.vtt.builder does not expose build_offers(...) "
            "or build_offer_from_raw(...)."
        )
    ...
    for raw in raw_rows or []:
        if not isinstance(raw, dict):
            continue
        offer = _BUILD_OFFER_FROM_RAW(raw, id_prefix=id_prefix)
...
def _build_offers_for_index(cfg, index: list[dict[str, Any]], *, id_prefix: str) -> list[OfferOut]:
    ...
    return _batch_build_offers(raw_rows, id_prefix=id_prefix)
