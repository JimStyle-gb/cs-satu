...
Что делает:
- нормализует title / vendor / model / price / availability;
- строит стабильный oid с префиксом AS;
- держит только supplier-base cleanup без final rendering;
- не содержит source-crawl и quality-gate логики.

Важно:
- все transforms здесь должны быть supplier-specific и rollback-safe;
- shared core не должен дублировать эту нормализацию.
"""
